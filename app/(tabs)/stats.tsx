...
export default function StatsScreen() {
  ...
  const leaks = allLeaks.filter((stat) => stat.providerId === provider);
  ...
  const previewNode = (leaks[0] && getNode(leaks[0].providerId, leaks[0].nodeId)) || allNodes(provider)[0];
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.tiles}>
        ...
        <StatTile label="Средняя потеря EV" value={evHands === 0 ? '—' : overallEv.toFixed(2)} color={colors.gold} sub={evHands === 0 ? 'Нет EV в источнике' : 'bb'} />
      </View>
      ...
      {leaks.length === 0 ? (
        <Card>
          <AppText color={colors.muted}>
            Данных пока нет. Пройдите несколько тренировок — здесь появятся споты с наивысшим приоритетом повторения.
          </AppText>
        </Card>
      ) : (
        leaks.slice(0, 6).map((stat, i) => <LeakCard key={`${stat.providerId}:${stat.nodeId}`} stat={stat} rank={i + 1} onDrill={() => drillNode(stat)} onView={() => router.push(`/heatmap/${stat.nodeId}?provider=${stat.providerId}`)} />)
      )}
      ...
      <AppText variant="caption" color={colors.muted}>
        Активный набор: {providerLabel} · спотов: {allNodes(provider).length}
      </AppText>
      {previewNode ? (
        <Pressable onPress={() => router.push(`/heatmap/${previewNode.id}?provider=${previewNode.providerId}`)}>
...
function LeakCard({ stat, rank, onDrill, onView }: { stat: NodeStat; rank: number; onDrill: () => void; onView: () => void }) {
  const node = getNode(stat.providerId, stat.nodeId);
  const acc = accuracy(stat);
  const severity = Math.max(0.05, Math.min(1, (100 - acc) / 100)); // longer red bar = worse
  const metric = `${acc} GTO · приоритет повторения`;
  return (
  ...
  return (
    <View style={styles.spark}>
      {recent.map((v, i) => {
        ...
        return <View key={i} style={{ width: 5, height: h, backgroundColor: c, borderRadius: 2 }} />;
      })}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  scoreCard: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(39,224,161,0.4)',
    padding: spacing.lg,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  leakTrack: { height: 6, backgroundColor: 'rgba(255,107,107,0.15)', borderRadius: radius.pill, overflow: 'hidden' },
