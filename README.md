...
Web-версия: https://mark3543634.github.io/preflop-trainer/

Лендинг и APK-раздел: https://mark3543634.github.io/preflop-trainer/landing/

Бесплатный офлайн-тренажёр префлопа: песочница для самостоятельных drills и
режим чтения диапазонов на флопе. Приложение написано на React Native,
...
Pekarstas импортируется из raw JSON без потери многоцветных ячеек и исходных
высот: условные советы больше не превращаются в выдуманные 50/50. Для
`vs_3bet` и `vs_4bet` генератор раздаёт только руки, которые достигают узла через
родительский диапазон. Проценты community-чартов обозначаются как совпадение с
чартом, а RNG применяется только к будущим подтверждённым solver-частотам.
...
- чтение диапазонов: последовательность preflop-действий, флоп и сравнение equity;
- очередь ошибок по интервалам 1/3/7 дней;
- provider-aware presets, review и per-node stats;
- интерфейс без игровых очков, серий, лиг и пользовательских рейтингов;
...
Workflow `Сайт` собирает Expo Web и публикует тренажёр в корне GitHub Pages.
Web-версия использует те же экраны, движок и локальное хранилище, что мобильное
приложение; отдельной урезанной реализации нет.
Статический русский лендинг из `site/` доступен по `/landing/` и автоматически
подставляет APK, версию и SHA-256 из последнего GitHub Release. До появления APK
кнопка честно показывает «APK готовится».
